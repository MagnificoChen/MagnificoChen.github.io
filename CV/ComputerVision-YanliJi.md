...
如图可见，高比特位（对应低频）存储主要形状，低比特位（对应高频）存储细节信息。
...
- 随机选取图像中的一个像素作为种子像素，并将其表示出来，如：label 1
- 检索种子附近（8 个）的未被标记的像素点，如果他们的差值在所规定的阈值内（阈值需要根据不同的情况进行设置），则合并到分割区域中
- 重复 b 步骤，直到区域停止扩张，并在此时再次随机选择非选定区域的一个像素做为种子像素
...
- 使用质心来表示大量的集群
- 层次聚类，如果需要，我们可以在过程中随时更改聚类数
- 分区聚类，在我们开始这个过程之前，我们要先确定需要的聚类数量
...
- 层次聚类的过程（凝聚型）
  ...
  2. 将距离最小的两个类合并成一个新类；
  3. 重新计算新类与所有类之间的距离；
  4. 重复 2、3，直到所有类最后合并成一类/聚类数达到目标。

- 层次聚类的过程（分裂型）
  1. 将所有对象看作一类
  2. 将距离最大的点分裂出来，形成一个新类
  3. 重新计算新类与所有类之间的距离，划分类别
  4. 重复步骤 2、3，直到聚类数达到目标
...
图像的灰度空间很像地球表面的整个地理结构，每个像素的灰度值代表高度。其中的灰度值较大的像素连成的线可以看做山脊，也就是分水岭。其中的水就是用于二值化的 gray threshold level，二值化阈值可以理解为水平面，比水平面低的区域会被淹没，刚开始用水填充每个孤立的山谷(局部最小值)。

当水平面上升到一定高度时，水就会溢出当前山谷，可以通过在分水岭上修大坝，从而避免两个山谷的水汇集，这样图像就被分成 2 个像素集，一个是被水淹没的山谷像素集，一个是分水岭线像素集。最终这些大坝形成的线就对整个图像进行了分区，实现对图像的分割。

算法过程：

- 把梯度图像中的所有像素按照灰度值进行分类，并设定一个测地距离阈值。
- 找到灰度值最小的像素点（默认标记为灰度值最低点），让 threshold 从最小值开始增长，这些点为起始点。
...
图像的重复性：例如，在路标中，不同地点的路标图案相似。我们可以通过特征点来实现：
...
检测可重复且独特的点，也就是：与图像变换无关的点，图像变换包括：
...
1. 检测：寻找一些鲜明的关键点
2. 描述：将这个点的特征以向量的形式提取出来
3. 匹配：计算特征向量之间的距离，寻找相似性。

![](57.png)

好的特征有一些特性：

1. 可重复性：我们可以在许多图像中找到这样的特征，且特征经过图像变换后依然可识别。
2. 显著性：每个特征都是易于识别的
3. 紧致有效：特征数量应该比像素数量少得多
4. 局部性：一个特征只占据图像的很小一块区域，对杂波和遮挡有健壮性

如下图，我们寻找的特征点就不能形成匹配，尽管两张图显然有相似性。

也就是，特征点需要有**关键点不变性**、**几何和光线差异不变性**。
...
1. 检测：
   我们不知道该特征最终会匹配到哪些其他图像位置，但是我们可以计算⼀个位置在外观上相对于位置 **u** 的微小变化有多稳定。将图像块与局部相邻处进行比较。

   **角点检测**
   我们可能会通过⼀个⼩窗⼝观察来识别该点；同时我们希望在任何⽅向上移动⼀个窗⼝，会产生较大的像素值强度变化。如下图
   ...
   Harris 角点检测法使用特征值的方式，使得任何方向上的角点都可以被检测出来。

   $$S(u,v) = \sum_{x,y}\omega(x,y)(I(x+u,y+v)-I(x,y))^2$$

   在这里， $\omega(x,y)$ 是加权函数/窗口函数， $I(x+u,y+v)$ 是像素 $(x,y)$ 周围的亮度值， $I(x,y)$ 是像素 $(x,y)$ 周围的像素值。整体的函数值代表了滑动前与滑动后对应的窗口中的像素点灰度变化

   算法步骤：

   - 计算窗口中各像素点在 x 和 y 方向的梯度；
   - 计算两个方向梯度的乘积,即 $Ix ^ 2$ , $Iy ^ 2$ , $IxIy$ (可以用一些一阶梯度算子求得图像梯度)
   - 使用滤波核对窗口中的每一像素进行加权，生成矩阵 M 和元素 A，B，C
   - 计算每个像素的 Harris 响应值 R，并对小于某阈值 T 的 R 置 0；
   - 由于角点所在区域的一定邻域内都有可能被检测为角点，所以为了防止角点聚集，最后在 3×3 3×3 或 5×5 5×5 的邻域内进行非极大值抑制，局部最大值点即为图像中的角点。
   ...
   - 不变性：图像发生变换，角点位置不变
   - 协变性：如果我们有两张同一图像的转换后的图，那么特征点应该在对应位置都能检测出来。

2. 描述：
   图像表示方法：直方图
   表示特征分布的全局直方图（颜色、纹理、深度...）、以及每个检测点的局部直方图。

   我们计算直方图是为了甚么？我们计算局部的定向梯度直方图，为了进行 **SIFT**，也就是尺度不变特征变换。

   #### SIFT

   成像匹配的核心问题是将同一目标在不同时间、不同分辨率、不同光照、不同位姿情况下所成的像相对应。传统的匹配算法往往是直接提取角点或边缘，对环境的适应能力较差，急需提出一种鲁棒性强、能够适应不同光照、不同位姿等情况下能够有效识别目标的方法。张三总结了现有的基于不变量技术的特征检测方法，并正式提出了一种基于尺度空间的、对图像缩放、旋转甚至仿射变换保持不变性的图像局部特征描述算子－SIFT（尺度不变特征变换），这种算法在 2004 年被加以完善。

   算法思想：**将一幅图像映射（变换）为一个局部特征向量集；特征向量具有平移、缩放、旋转不变性，同时对光照变化、仿射及投影变换也有一定不变性。**
   ...
   - 对关键点附加详细的信息（局部特征）也就是所谓的描述器；
     我们在 16x16 的窗口进行检测，根据已有的方向和缩放指数，旋转和缩放窗口（构建不变性）；计算高斯加权后的梯度（平滑衰减）；在每个 4x4 的 1/16 象限中，通过加权梯度值加到直方图 8 个方向区间中的一个，计算出一个梯度方向直方图。这样就可以对每个特征形成一个 4x4x8=128 维的描述子，每一维都可以表示 4x4 个格子中一个的缩放/方向。将这个向量归一化之后，就进一步去除了光照的影响。
