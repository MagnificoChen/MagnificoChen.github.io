...
$$
a=f(x)\\ \textbf{\rm{if}}\ (a>0)\ \textbf{\rm{then}} \\
  \rm{do\ something}
$$
...
其中 $\textbf{\rm{p}}=(x,y)$ ,c 是某点 $(x_c,y_c)$ 。

更直观的，那俩平方其实就是距离：
$$\parallel\textbf{\rm{p}}-\textbf{\rm{c}}\parallel^2-r^2 = 0$$
...
> shading和shadow不是一个东西。在我们考虑着色时，我们不考虑其他物体挡住光的情形。
...
> **半程向量 h**是通过对向量 l 和向量 v 取平均然后归一化得到的一个新向量。这样口语简化计算。
