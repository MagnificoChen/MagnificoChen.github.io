...
![图 4-1](images/4/4-1.png "我们的”视角光线“进入场景，第一个碰到的是三角形T2，然后返回了")
...
![图 4-3](images/4/4-3.png "当投影出来的线和图像平面平行或垂直，称为正交投影。")
...
![图 4-5](images/4/4-5.png "在三点透视中，我们选择了平行线相交的三个点。每条轴向的平行线都有自己的交汇点。")
...
![图 4-6](images/4/4-6.png "从眼睛到图像上某一点的“光线”")
...
![图 4-7](images/4/4-7.png "图像上的二维坐标映射到了三维中，我们给每个像素射出一条光线做追踪。")
...
![图 4-8](images/4/4-8.png "相机帧的局部坐标向量。w向量与视角方向相反，v向量和 向上向量和w向量构成的平面 共面。")
...
![图 4-9](images/4/4-9.png "通过相机帧来生成光线。左图：光线从每个像素处出发，方向均相同；右图：透视投影，每条光线从视点发射，到像素点停下。")
...
![图 4-12](images/4/4-12.png "兰伯特光照的几何模型")
...
![图 4-13-15](images/4/4-13-15.png "左：单光源漫反射  中：三光源漫反射+阴影   右：蓝球是漫反射，绿球是冯着色，三光源")
...
![图 4-16](images/4/4-16.png "布林-冯反射的几何模型")
...
![图 4-17](images/4/4-17.png "p点不在阴影中，q在。")
...
![图 4-19](images/4/4-19.png "在完美镜面中，视角向量d会被镜面反射成向量r。")
...
![图 4-20](images/4/4-20.png "使用布林-冯反射和漫反射，加上三个点光源，渲染阴影，地面有镜面反射的效果")
...
![图 6-1](images/6/6-1.png "每个轴缩小一半，是等比缩放")
...
![图 6-2](images/6/6-2.png "x轴缩小一半，y轴增大一半，不是等比缩放")

### 6.1.2 切变

切变是一种把东西推到一边的变换。它能产生的效果就像用手推一副扑克牌，让他变得倾斜。水平和垂直方向的切变如下所示：

![图 6-3](images/6/6-3.png)
![图 6-4](images/6/6-4.png)
...
![图 6-6](images/6/6-6.png "旋转45度，是逆时针旋转。")
...
![图 6-8](images/6/6-8.png "将x轴坐标取反，得到沿y轴的镜像") ![图 6-9](images/6/6-9.png "将y轴坐标取反，得到沿x轴的镜像")
...
![图 6-10](images/6/6-10.png "按顺序做变换。和直接乘上一个算好的矩阵是一样的效果。")

实际中，用旋转和缩放来进行变换已经可以覆盖所有的 _线性变换_ 。这一点很重要，下一节我们会着重讨论。

![图 6-11](images/6/6-11.png "小心！变换的施加顺序很重要，交换顺序会导致不同的结果。")
...
![图 6-12](images/6/6-12.png "使用奇异值分解来对付切变矩阵：每个变换矩阵都可以视为旋转+缩放+旋转的组合。")
...
![图 6-13](images/6/6-13.png "单位圆被对称矩阵作用的效果（这个对称矩阵起的作用不是轴向的；且不是等比缩放）")
...
![图 6-14](images/6/6-14.png "对称矩阵的作用永远是向着一个轴进行缩放。这个例子中，轴是31.7度，说明矩阵的特征向量朝着这个方向。")
...
![图 6-15](images/6/6-15.png "这里A是任意的矩阵。实际上，v1和v2是A的右奇异向量，它们被转换成左奇异向量的样子，也就是u1和u2。我们对这个变换做因式分解，可以分解成：将右奇异向量旋转成标准轴向，然后做轴向的缩放，再将其转到左奇异向量的位置。")
...
![图 6-16](images/6/6-16.png "所有二维旋转都可以分解成三个切变操作。这个例子是旋转45度，步骤如下。")
...
![图 6-19](images/6/6-19.png "点 (2,1) 经过了左移-1的变换。右上角是物理移动的样子，右下角是坐标轴移动（移动原点）的样子。")
...
![图 6-20](images/6/6-20.png "两种坐标系统都能表示p点。")
