...
如果我们有一个函数 $f:A\to B$ ,那么就可能存在一个 _反函数_ : $f^{-1}:B\to A$ ，仅在所有的 $b\in B$ 都是值域中的一个点，而且此点唯一对应（也就是每个 $a$ 只对应一个 $b$ ）时，取逆操作成立。这样的映射称作 *双射* 。一个双射中，每一个 $a$ 都对应一个唯一的 $b$ ，而且对于每一个 $b$ 都有一个 $a$ 与之对应。不是双射的映射当然就没有逆映射这个概念。
