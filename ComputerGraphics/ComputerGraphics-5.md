...
层次包围盒的基本思想是在所有对象外圈放置一个与 **轴对齐** 的三维包围盒，如 [图12.25] 所示。实际上-，与暴力搜索相比，击中边界框的光线的计算成本更高，因为测试与边界框的交集不是免费的。然而，错过盒子的光线比暴力搜索便宜。这样的边界框可以通过划分边界框Ray d y = ymax y = ymin x = xmin x = xmin e来分层。2D射线e + t d针对2D边界框进行测试。

如图12.26所示，在每个分区周围放置一个方框。

图12.27所示层次结构的数据结构可能是一个树，大的边界框位于根，两个较小的边界框分别作为左子树和右子树。这些点依次指向三个三角形的列表。射线与这个特定的硬编码树的交点是: