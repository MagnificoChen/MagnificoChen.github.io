...
```
![图12-32](./images/12/12-33.png)
![图12-32](./images/12/12-34.png)
...
![图12-32](./images/12/12-37.png)
![图12-32](./images/12/12-38.png)
...
有效地利用内存层次结构是现代体系结构算法设计中的一项关键任务。通过平铺（有时也称为铺砖）来确保多维数组的数据以“良好”的方式排列。传统的二维数组会被存储为一维数组，并带有索引机制；例如， $N_x × N_y$ 大小的数组存储在长度为 $NxNy$ 的一维数组中，同时我们将索引 $(x, y)$ （从 $(0,0)$ 到 $(N_x−1,N_y−1)$ ）通过如下公式映射成一维索引（从 $0$ 到 $N_xN_y−1$ ）：
...
一个关键的问题是 *铺砖* 的尺寸。在实际使用中，铺砖的尺寸应该与机器上的 **内存单元大小** 相似。例如，如果我们在具有 128B *缓存行* 的机器上使用16位（2Byte）的数据值，则划分成 $8×8$ 的块正好适合缓存行大小。但是，如果使用32位浮点数（一条缓存行上可以容纳32个元素），$5×5$ 块就有点太小了， $6×6$ 块又有点太大了（需要32）。因为也有更粗粒度的内存单元（内存页），所以具有类似逻辑的 *分层平铺* 就可能派上用场。

### 12.5.1 二维数组的单层平铺

如果我们假设一个 $N_x×N_y$ 大小的数组需要被平铺，铺砖大小是正方形的 $n×n$ 块 [图12-42] ，那么所需的块数为
