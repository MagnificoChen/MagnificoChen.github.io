...
$\mathbf{p}_i$ 称作 _系数_ ， $b_i$ 称作 _基函数_ 。如果系数是以点的形式（比如二维向量、三维向量等）表示，它就叫 **控制点** 。为了实现B样条的特性，需要我们好好选择基函数 $b_i$ 。B 样条的基函数选择方法非常通用。
...
> 插值了端点的、控制点个数 $n=8$ 的三次B样条。节点向量是 $[0,0,0,1,2,3,4,5,6,6,6]$ 。头两段和末两段B样条是非周期（非均匀）的，而中间的四段（虚线）B样条是周期的（均匀的），类似于 [图15-20] 中的B样条基函数曲线。
